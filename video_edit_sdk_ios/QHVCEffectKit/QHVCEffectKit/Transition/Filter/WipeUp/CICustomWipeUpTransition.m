...
#import "CICustomWipeUpTransition.h"
#import "QHVCEffectUtils.h"

NSString* const customWipeUpTransitionKernelString = KERNEL_STRING
(
 vec4 transition(sampler inputImage1, sampler inputImage2, float progress)
{
    ...
    return mix(color1, color2, step(p.y,progress));
}
...
static CIKernel* customWipeUpTransitionKernel = nil;
@implementation CICustomWipeUpTransition
...
- (instancetype)init
{
    ...
    if (self)
    {
        if (!customWipeUpTransitionKernel)
        {
            customWipeUpTransitionKernel = [CIKernel kernelWithString:customWipeUpTransitionKernelString];
        }
    }
...
- (CIImage *)outputImage
{
    CIImage* result = [customWipeUpTransitionKernel applyWithExtent:_inputImage.extent
