...
- (void)onRestoreAction
{
    ...
    {
        [obj.clip setSourceRadian:_initialRadian];
        [obj.clip setFlipX:_initialFlipX];
        [obj.clip setFlipY:_initialFlipY];
    }];
    
    QHVCEditSequenceTrack* mainTrack = [[QHVCEditMediaEditor sharedInstance] getMainVideoTrack];
    [mainTrack setFillMode:kDefaultFillModeIndex];
    [mainTrack setBgParams:nil];
...
- (void)onRotateAction
{
    ...
    [_clips enumerateObjectsUsingBlock:^(QHVCEditTrackClipItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
    {
        [obj.clip setSourceRadian:radian];
    }];
    SAFE_BLOCK(self.refreshPlayerBlock, YES);
}
