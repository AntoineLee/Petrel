...
@property (nonatomic, retain) QHVCEditFrameView* thumbnailContentView;
...
- (void)prepareSubviews
{
    ...
    self.thumbnailContentView = [[NSBundle mainBundle] loadNibNamed:@"QHVCEditFrameView" owner:self options:nil][0];
    ...
    WEAK_SELF
    [self.thumbnailContentView setPlayAction:^{
        STRONG_SELF
        SAFE_BLOCK(self.playPlayerBlock);
    }];
    
    [self.thumbnailContentView setPauseAction:^{
        STRONG_SELF
        SAFE_BLOCK(self.pausePlayerBlock);
    }];
    
    [self.thumbnailContentView setSeekAction:^(BOOL forceRefresh, NSInteger seekToTime)
    {
        STRONG_SELF
