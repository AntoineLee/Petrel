...
- (void)viewDidLoad
{
    ...
    //文件存入沙盒
    WEAK_SELF
    [[QHVCPhotoManager manager] writeAssetsToSandbox:self.resourceArray complete:^{
        STRONG_SELF
...
- (void)fetchThumbs
{
    ...
    [[QHVCEditMediaEditor sharedInstance] requestThumbs:clip.filePath start:0 end:clip.durationMs frameCnt:kThumbTotolCount thumbSize:CGSizeMake(100, 200) thumbCallback:^(QHVCEditThumbnailItem *thumbnail)
     {
...
{
    [self freePlayer];
    [[NSNotificationCenter defaultCenter] postNotificationName:QHVCEDIT_DEFINE_NOTIFY_CLEAN_ALL_PARAMS object:nil];
