...
@interface QHVCEditFrameView() <UIScrollViewDelegate>
{
            ...
            [self.clipArray enumerateObjectsUsingBlock:^(QHVCEditTrackClipItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
             {
                 if (insertTime <= time && (insertTime + obj.clip.duration) >= time)
                 {
                     ...
                     [[QHVCEditMediaEditor sharedInstance] requestThumb:obj.filePath timestamp:realTime thumbSize:CGSizeMake(100, 200) thumbCallback:^(QHVCEditThumbnailItem *thumbnail)
                      {
...
{
    SAFE_BLOCK(self.seekAction, YES, self.currentTime);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.isDraging)
    {
        SAFE_BLOCK(self.seekAction, NO, self.currentTime);
    }
}
